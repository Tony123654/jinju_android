...
public class PullUpToLoadMore extends ViewGroup {
    ...
    MyDetailScrollView topScrollView, bottomScrollView;
    ...
    private void init(Context context) {
        ...
        post(new Runnable() {
            @Override
            public void run() {
                topScrollView = (MyDetailScrollView) getChildAt(0);

                bottomScrollView = (MyDetailScrollView) getChildAt(1);

                topScrollView.setScrollListener(new MyDetailScrollView.ScrollListener() {
                    @Override
                ...
                bottomScrollView.setScrollListener(new MyDetailScrollView.ScrollListener() {
                    @Override
                    public void onScrollToBottom() {
                    }
                ...
                if (bottomScrollVIewIsInTop) {
                    ...
                    if (dy < 0 && currPosition == 1) {
                        if (Math.abs(dy) >= scaledTouchSlop) {
                            if (PublicStaticClassUtils.IsTop) {//如果viewpager里边的scrollview在最顶部，，就让外边的scrollview获取焦点，否则，让最里边的scrollview获取焦点
                                isIntercept = true;
                            }
                        }
                    }
