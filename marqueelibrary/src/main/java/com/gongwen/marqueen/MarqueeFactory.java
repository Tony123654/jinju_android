...
public abstract class MarqueeFactory<T extends View, E> {
    ...
    //适用于多次（含一次）更新数据源
    public void resetData(final List<E> datas,boolean startingAnimation) {
        //startingAnimation表示是否正在动画
        if (datas == null || datas.size() == 0) {
            return;
        }
        if (mMarqueeView == null || (mMarqueeView != null && this.datas == null)||!startingAnimation) {
            setData(datas);
        } else {
            //防止多次更新数据可能导致的叠影问题
